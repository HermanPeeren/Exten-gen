 ...
 *
 * Each row of the projects list carries its own generate URL in
 * `data-href`, and there is one modal for the whole page. So the click has to
 * say *which* project before the dialog appears, and that is all this does.
 *
 * It was twelve lines inline in `tmpl/projects/default.php` with a note
 * asking for exactly this. Moving it out is not tidiness: inline script in a
 * template is script no test can import, and this is the same twelve lines
 * Meta-gen had, with the same note on them. Both were extracted at once, and
 * the two files are deliberately identical: the next library release should
 * take this into `lib_yepr_gen` beside `reference.js` and let both components
 * drop their copy.
 *
