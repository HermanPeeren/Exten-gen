/**
 * <extengen-reference>: a dropdown that knows what the form currently holds.
 *
...
/**
 * What the page carries: the stored project's references, and a description of
 * each object type. Both come from `ReferenceIndex` on the server, so there is
 * one statement of what an Entity or a Field is rather than two.
 */
function pageData() {
  const options = window.Joomla && typeof window.Joomla.getOptions === 'function'
    ? window.Joomla.getOptions('com_extengen.references')
    : null;
 ...
 */
class ExtengenReference extends HTMLElement {
  connectedCallback() {
...
if (!window.customElements.get('extengen-reference')) {
  window.customElements.define('extengen-reference', ExtengenReference);
}
...
document.addEventListener('change', (event) => {
  ...
  for (const element of document.querySelectorAll('extengen-reference')) {
    element.refresh();
  }
});

export { ExtengenReference };
