 ...
 */
class MetalanguageTable extends Table implements TaggableTableInterface
{
	...
	{
		$this->typeAlias = 'com_extengen.metalanguage';

		parent::__construct('#__extengen_metalanguage', 'id', $db);
	}
...
/*
	...
	/**
	 * Method to delete a metalanguage (and mappings of that metalanguage to related entities) from the database.
	 *
	 * @param   integer  $metalanguageId  An optional metalanguage id.
	 *
	 * @return  boolean  True on success, false on failure.
	 *
	 * @see     \Joomla\CMS\Table\User handling of user groups
	 */
	public function delete($metalanguageId = null):bool
	{
		$db    = $this->getDatabase();
		...
		if ($metalanguageId) {
			...
			$this->$k = (int) $metalanguageId;
		}

		$key = (int) $this->$k;


		// Delete the metalanguage.
		$query->clear()
