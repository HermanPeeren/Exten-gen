...
namespace Yepr\Component\eventschedule\Administrator\View\Tracks;
...
use Yepr\Component\eventschedule\Administrator\Model\TracksModel;
...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar()
	{
		...
		ToolbarHelper::title(Text::_('COM_EVENTSCHEDULE_MANAGER_TRACKS'), 'address tracks');
		...
		if ($canDo->get('core.edit.state'))
		{
			...
			$childBar->standardButton('featured')
				->text('JFEATURE')
				->task('tracks.featured')
				->listCheck(true);
			$childBar->standardButton('unfeatured')
				->text('JUNFEATURE')
				->task('tracks.unfeatured')
				->listCheck(true);
			...
			{
				$childBar->checkin('tracks.checkin')->listCheck(true);
			}

			if ($this->state->get('filter.published') != -2)
			{
				$childBar->trash('tracks.trash')->listCheck(true);
			}

			if ($this->state->get('filter.published') == -2 && $canDo->get('core.delete'))
			{
				$childBar->delete('tracks.delete')
