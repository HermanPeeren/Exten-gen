...
/**
 * Methods supporting a list of Presentations records.
 */
class PresentationsModel extends ListModel
{
    /**
	 * Constructor.
	 *
	...
	protected function getListQuery():QueryInterface
	{
		...
		$query->select([
			$db->quoteName('presentation.presentation_name'),
			$db->quoteName('presentation.short_description'),
			$db->quoteName('presentation.long_description'),
			$db->quoteName('presentation.duration'),
			$db->quoteName('presentation.id')
		]);
		$query->from($db->quoteName('#__eventschedule_presentation', 'presentation'));

		// Add the presentation fields from the joined tables
		$query->select($db->quoteName('presentation_type.presentation_type_name', 'presentation_type'));
		$query->join(
			'LEFT',
