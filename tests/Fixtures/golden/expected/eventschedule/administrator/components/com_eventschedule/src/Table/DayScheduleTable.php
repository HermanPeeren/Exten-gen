...
/**
 * DaySchedule Table class.
 */
class DayScheduleTable extends Table implements TaggableTableInterface
{
	...
	{
		$this->typeAlias = 'com_eventschedule.dayschedule';

		parent::__construct('#__eventschedule_dayschedule', 'id', $db);
	}
...
/*
	...
	/**
	 * Method to delete a dayschedule (and mappings of that dayschedule to related entities) from the database.
	 *
	 * @param   integer  $dayscheduleId  An optional dayschedule id.
	 *
	 * @return  boolean  True on success, false on failure.
	 *
	 * @see     \Joomla\CMS\Table\User handling of user groups
	 */
	public function delete($dayscheduleId = null):bool
	{
		$db    = $this->getDatabase();
		...
		if ($dayscheduleId) {
			...
			$this->$k = (int) $dayscheduleId;
		}

		$key = (int) $this->$k;


		// Delete the dayschedule.
		$query->clear()
