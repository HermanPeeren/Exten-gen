...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar()
	{
		...
		ToolbarHelper::title(Text::_('COM_EVENTSCHEDULE_MANAGER_SCHEDULE'), 'address schedule');
		...
		if ($canDo->get('core.edit.state'))
		{
			...
			$childBar->publish('schedule.publish')->listCheck(true);
			$childBar->unpublish('schedule.unpublish')->listCheck(true);

			$childBar->standardButton('featured')
				->text('JFEATURE')
				->task('schedule.featured')
				->listCheck(true);
			$childBar->standardButton('unfeatured')
				->text('JUNFEATURE')
				->task('schedule.unfeatured')
				->listCheck(true);

			$childBar->archive('schedule.archive')->listCheck(true);

			if ($user->authorise('core.admin'))
			{
				$childBar->checkin('schedule.checkin')->listCheck(true);
			}

			if ($this->state->get('filter.published') != -2)
			{
				$childBar->trash('schedule.trash')->listCheck(true);
			}

			if ($this->state->get('filter.published') == -2 && $canDo->get('core.delete'))
			{
				$childBar->delete('schedule.delete')
