...
namespace Yepr\Component\Conference\Administrator\View\Program;
...
use Yepr\Component\Conference\Administrator\Model\ProgramModel;

/**
 * View class for a list of Program.
 */
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar()
	{
		...
		ToolbarHelper::title(Text::_('COM_CONFERENCE_MANAGER_PROGRAM'), 'address program');
		...
		if ($canDo->get('core.edit.state'))
		{
			...
			$childBar->publish('program.publish')->listCheck(true);
			$childBar->unpublish('program.unpublish')->listCheck(true);

			$childBar->standardButton('featured')
				->text('JFEATURE')
				->task('program.featured')
				->listCheck(true);
			$childBar->standardButton('unfeatured')
				->text('JUNFEATURE')
				->task('program.unfeatured')
				->listCheck(true);

			$childBar->archive('program.archive')->listCheck(true);

			if ($user->authorise('core.admin'))
			{
				$childBar->checkin('program.checkin')->listCheck(true);
			}

			if ($this->state->get('filter.published') != -2)
			{
				$childBar->trash('program.trash')->listCheck(true);
			}

			if ($this->state->get('filter.published') == -2 && $canDo->get('core.delete'))
			{
				$childBar->delete('program.delete')
