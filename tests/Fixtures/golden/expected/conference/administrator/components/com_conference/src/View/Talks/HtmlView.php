...
namespace Yepr\Component\Conference\Administrator\View\Talks;
...
use Yepr\Component\Conference\Administrator\Model\TalksModel;

/**
 * View class for a list of Talks.
 */
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar()
	{
		...
		ToolbarHelper::title(Text::_('COM_CONFERENCE_MANAGER_TALKS'), 'address talks');
		...
		if ($canDo->get('core.edit.state'))
		{
			...
			$childBar->publish('talks.publish')->listCheck(true);
			$childBar->unpublish('talks.unpublish')->listCheck(true);
			...
			$childBar->standardButton('unfeatured')
				->text('JUNFEATURE')
				->task('talks.unfeatured')
				->listCheck(true);

			$childBar->archive('talks.archive')->listCheck(true);

			if ($user->authorise('core.admin'))
			{
				$childBar->checkin('talks.checkin')->listCheck(true);
			}

			if ($this->state->get('filter.published') != -2)
			{
				$childBar->trash('talks.trash')->listCheck(true);
			}

			if ($this->state->get('filter.published') == -2 && $canDo->get('core.delete'))
			{
				$childBar->delete('talks.delete')
