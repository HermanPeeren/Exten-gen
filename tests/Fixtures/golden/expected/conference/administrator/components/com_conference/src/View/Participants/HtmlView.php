...
namespace Yepr\Component\Conference\Administrator\View\Participants;
...
use Yepr\Component\Conference\Administrator\Model\ParticipantsModel;
...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar()
	{
		...
		ToolbarHelper::title(Text::_('COM_CONFERENCE_MANAGER_PARTICIPANTS'), 'address participants');
		...
		{
			$toolbar->addNew('participant.add');
		}
		...
		if ($canDo->get('core.edit.state'))
		{
			...
			$childBar->publish('participants.publish')->listCheck(true);
			$childBar->unpublish('participants.unpublish')->listCheck(true);

			$childBar->standardButton('featured')
				->text('JFEATURE')
				->task('participants.featured')
				->listCheck(true);
			$childBar->standardButton('unfeatured')
				->text('JUNFEATURE')
				->task('participants.unfeatured')
				->listCheck(true);

			$childBar->archive('participants.archive')->listCheck(true);

			if ($user->authorise('core.admin'))
			{
				$childBar->checkin('participants.checkin')->listCheck(true);
			}

			if ($this->state->get('filter.published') != -2)
			{
				$childBar->trash('participants.trash')->listCheck(true);
			}
