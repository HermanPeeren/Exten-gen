...
class ProgramModel extends ListModel
{
	...
	protected function getListQuery()
	{
		...
		{
			$query
				->where($db->quoteName('program.room_id') . ' LIKE '
					. $db->quote($room));
		}
		...
		{
			$query
				->where($db->quoteName('program.talk_id') . ' LIKE '
					. $db->quote($talk));
		}
		...
		{
			$query
				->where($db->quoteName('program.time') . ' LIKE '
					. $db->quote($time));
		}
		...
		{
			$query
				->where($db->quoteName('program.title') . ' LIKE '
					. $db->quote($title));
		}
