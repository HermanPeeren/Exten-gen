...
use Yepr\Component\Extengen\Administrator\Generator\Meta\LanguageStructure;
use Yepr\Component\Extengen\Administrator\Generator\Meta\ReferenceTable;
 ...
 * knowledge of it. `tests/Fixtures/projectforms/lioncore-m3.json` is LionCore M3
 ...
 * `ProjectFormReferenceTest` uses, so the two tests describe one thing.
 *
 * @since  1.2.0
 */
final class MetaReferenceTableTest extends TestCase
{
    ...
    private function metaModel(): ConceptModel
    {
        return ConceptModel::fromJson(
            (string) file_get_contents(\dirname(__DIR__) . '/Fixtures/projectforms/lioncore-m3.json')
        );
    }

    /**
     * A projectForm written in that language, as the component stores one.
     */
    private function storedModel(): object
    {
        $json = (string) file_get_contents(\dirname(__DIR__) . '/Fixtures/projectforms/er1.json');
    ...
    {
        $this->assertSame(
            ReferenceIndex::projectForm()->clientTypes(),
     ...
     *
     * The whole table, run over a stored projectForm: every type, every
    ...
    public function testItIndexesAStoredModelTheSameWay(): void
    {
        ...
        $this->assertSame(
            ReferenceIndex::projectForm()->index($stored),
    ...
    public function testALanguageShapedLikeEr1ReproducesTheOtherHandWrittenTable(): void
    {
        ...
        // And the whole client half against the hand-written one, rather than
        // against what is written out above: a transcribed expectation only
        // proves the transcription.
        $this->assertSame(
            ReferenceIndex::project()->clientTypes(),
            ReferenceIndex::fromTable($generated)->clientTypes()
        );
    }
    ...
    public function testALanguageWithNoTreeYetIsReportedRatherThanIndexed(): void
    {
        $sketch = ConceptModel::fromJson(
            (string) file_get_contents(\dirname(__DIR__) . '/Fixtures/projectforms/er1.json')
        );
