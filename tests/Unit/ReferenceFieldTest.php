 ...
 *
 * This is the seam: PHP writes `<extengen-reference type="Entity">` around a
 * `<select>`, and `admin-extengen-reference.js` reads those attributes to
 ...
 * id and a value out of a Joomla form, and this suite deliberately has no
 * Joomla in it. Everything that could be wrong about the markup is here.
 */
final class ReferenceFieldTest extends TestCase
{
    public function testItRendersTheCustomElementAroundARealSelect(): void
    {
        $html = $this->render('Entity', 'abc-123');

        $this->assertStringContainsString('<extengen-reference type="Entity">', $html);
        $this->assertStringContainsString('<select name="jform[reference][reference0][reference]"', $html);
        $this->assertStringContainsString('</extengen-reference>', $html);
    }
