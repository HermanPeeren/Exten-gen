 ...
 * the directory was `GenerateProjectForm`, and the class inside it declared
 * `View\GenerateForm` - so the button led nowhere, and underneath it a
 * generator that opened `foreach ($projectForm->datamodel as $entity)` would
 ...
 *
 * The projectForm is seeded by `tools/seed-projectform.php`, which is the same
...
describe('generating the forms of a language', () => {
  ...
  it('is reachable from the project forms list', () => {
    cy.visitExtengen('projectforms');
    cy.shouldHaveRendered();

    cy.get('#adminForm a[data-href*="view=generateProjectForm"]').first().then(($link) => {
  ...
  it('generates the forms and reports what it wrote', () => {
    cy.visit('/administrator/index.php?option=com_extengen&view=generateProjectForm'
      + '&tmpl=component&projectform_id=1');
   ...
   * `administrator/components/com_extengen/forms/ProjectForms/`, with a
   ...
   * left a language half replaced. The projectForm list still opening
  ...
  it('leaves the component it runs inside alone', () => {
    ...
    cy.visit('/administrator/index.php?option=com_extengen&task=projectform.edit&id=1');
    cy.shouldHaveRendered();
